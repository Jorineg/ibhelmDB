...
-- Last path segment of nas_folder_path (single folder name on NAS); NULL if empty
...
-- Lowercase + ä/ö/ü/ß folding for Craft path vs name matching
...
-- Link file to project if full_path contains effective storage folder (nas segment or Teamwork name)
...
-- Craft: (1) match folded Teamwork name in folder_path; else (2) match type_source_tag_name from task_extensions
...
-- OUT params changed (storage_folder_name): DROP required before replace
...
-- Distinct NAS root folder names from scanned files, ranked by pg_trgm similarity to Teamwork project name
